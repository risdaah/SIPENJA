...
async function simpanLayanan() {
    ...
    if (!payload.KODELAYANAN || !payload.NAMA) {
        Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Kode dan Nama layanan wajib diisi.' });
        return;
    }
    ...
    try {
        ...
        await loadLayanan();
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Layanan servis berhasil ditambahkan.', timer: 1800, showConfirmButton: false });
    } catch (err) {
...
async function updateLayanan() {
    ...
    try {
        ...
        await loadLayanan();
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Layanan servis berhasil diupdate.', timer: 1800, showConfirmButton: false });
    } catch (err) {
        Swal.fire({ icon: 'error', title: 'Gagal!', text: err.message });
    }
}
...
function konfirmasiHapus(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const res = await fetch(`${API_BASE_URL}/api/layanan-servis/delete/${id}`, { method: 'DELETE' });
                if (!res.ok) throw new Error('Gagal menghapus data');
                await loadLayanan();
                Swal.fire({ icon: 'success', title: 'Terhapus!', text: 'Layanan servis berhasil dihapus.', timer: 1800, showConfirmButton: false });
            } catch (err) {
                Swal.fire({ icon: 'error', title: 'Gagal!', text: err.message });
            }
        }
