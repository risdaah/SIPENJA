...
async function simpanKaryawan() {
    ...
    if (!payload.NAMA || !payload.USERNAME || !payload.PASSWORD || !payload.ROLE) {
        Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Nama, Username, Password, dan Role wajib diisi.' });
        return;
    }
    ...
    try {
        ...
        await loadKaryawan();
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Karyawan berhasil ditambahkan.', timer: 1800, showConfirmButton: false });
    } catch (err) {
...
function bukaModalEdit(id) {
    ...
    document.getElementById('editTanggalLahir').value = formatTanggalInput(item.TANGGALLAHIR);
...
async function updateKaryawan() {
    ...
    if (!payload.NAMA || !payload.USERNAME || !payload.ROLE) {
        Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Nama, Username, dan Role wajib diisi.' });
        return;
    }
    ...
    try {
        ...
        await loadKaryawan();
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Data karyawan berhasil diupdate.', timer: 1800, showConfirmButton: false });
    } catch (err) {
        Swal.fire({ icon: 'error', title: 'Gagal!', text: err.message });
    }
}
...
async function gantiPassword() {
    ...
    if (!pass) {
        Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Password baru wajib diisi.' });
        return;
    }
    if (pass !== konf) {
        Swal.fire({ icon: 'warning', title: 'Perhatian', text: 'Password dan konfirmasi tidak cocok.' });
        return;
    }
    ...
    try {
        ...
        Swal.fire({ icon: 'success', title: 'Berhasil!', text: 'Password berhasil diubah.', timer: 1800, showConfirmButton: false });
    } catch (err) {
        Swal.fire({ icon: 'error', title: 'Gagal!', text: err.message });
    }
}
...
function konfirmasiHapus(id) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const res = await fetch(`${API_BASE_URL}/api/user/delete/${id}`, { method: 'DELETE' });
                if (!res.ok) throw new Error('Gagal menghapus data');
                await loadKaryawan();
                Swal.fire({ icon: 'success', title: 'Terhapus!', text: 'Karyawan berhasil dihapus.', timer: 1800, showConfirmButton: false });
            } catch (err) {
                Swal.fire({ icon: 'error', title: 'Gagal!', text: err.message });
            }
        }
