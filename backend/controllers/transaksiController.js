...
// ── BARU: GET /transaksi/get-by-kasir/:idKasir?jenis=SERVIS ──────────────────
const getTransaksiByKasir = async (req, res) => {
  try {
    const idKasir = parseInt(req.params.idKasir, 10);
    if (isNaN(idKasir)) {
      return res.status(400).json({ success: false, message: "idKasir tidak valid" });
    }
...
const getTransaksiByDateRange = async (req, res) => {
  try {
    ...
    const data = await Transaksi.getByDateRange(
      startDate, endDate, jenis ? jenis.toUpperCase() : null,
    );
    res.json({ success: true, data });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
const getTransaksiByBulan = async (req, res) => {
  try {
    ...
    const data = await Transaksi.getByBulan(
      bulan, tahun, jenis ? jenis.toUpperCase() : null,
    );
    res.json({ success: true, data });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
...
const createTransaksi = async (req, res) => {
  try {
    const {
      ...
      CATATAN, NAMAPELANGGAN, KELUHAN, LAYANAN, ITEMS,
    } = req.body;
    ...
    if (JENISTRANSAKSI === "SERVIS") {
      ...
      const transaksi = await Transaksi.create({ IDUSER: IDUSER_KASIR, JENISTRANSAKSI, TOTAL: 0, CATATAN: CATATAN || null });
