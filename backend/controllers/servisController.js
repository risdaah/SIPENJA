...
const trackServis = async (req, res) => {
  try {
    ...
    if (!data) {
      return res.status(404).json({ success: false, message: 'Kode antrian tidak ditemukan. Pastikan kode antrian sudah benar.' });
    }
...
const updateServis = async (req, res) => {
  try {
    ...
    if (!NAMAPELANGGAN && !KELUHAN && CATATAN === undefined) {
      return res.status(400).json({ success: false, message: 'Minimal satu field (NAMAPELANGGAN, KELUHAN, atau CATATAN) harus diisi' });
    }
...
const updateProgress = async (req, res) => {
  try {
    ...
    const keterangan = KETERANGAN || `Status diupdate menjadi ${STATUS}`;
  ...
  try {
    ...
    for (const item of ITEMS) {
      if (!item.IDSPAREPART || !item.QTY) {
        return res.status(400).json({ success: false, message: 'IDSPAREPART dan QTY wajib diisi pada setiap item' });
      }
      if (item.QTY <= 0) {
  ...
  try {
    ...
    for (const item of ITEMS) {
      if (!item.IDLAYANANSERVIS) {
        return res.status(400).json({ success: false, message: 'IDLAYANANSERVIS wajib diisi pada setiap item' });
      }
    }
...
const updateLayanan = async (req, res) => {
  try {
    ...
    if (BIAYA === undefined && KETERANGAN === undefined) {
      return res.status(400).json({ success: false, message: 'Minimal BIAYA atau KETERANGAN harus diisi' });
    }
...
const updateSparepart = async (req, res) => {
  try {
    ...
    if (!QTY) {
      return res.status(400).json({ success: false, message: 'QTY wajib diisi' });
    }
    if (QTY <= 0) {
      return res.status(400).json({ success: false, message: 'QTY harus lebih dari 0' });
    }
