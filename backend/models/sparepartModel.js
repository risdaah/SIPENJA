...
const Sparepart = {
  ...
  create: async (data) => {
    ...
    const [lastRow] = await db.query(
      "SELECT MAX(IDSPAREPART) as lastId FROM SPAREPART",
    );
  ...
  update: async (id, data) => {
    ...
    await db.query(
     ...
     HARGAJUAL = ?, STOKMINIMUM = ? WHERE IDSPAREPART = ?`,
      [IDKATEGORI, IDSUPPLIER, KODESPAREPART, NAMA, HARGAJUAL, STOKMINIMUM, id],
    );
  ...
  updateStok: async (id, STOK) => {
    await db.query("UPDATE SPAREPART SET STOK = ? WHERE IDSPAREPART = ?", [
      STOK,
      id,
    ]);
